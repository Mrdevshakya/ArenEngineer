...
Aren extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
aren channels login --channel zalouser
...
```bash
aren message send --channel zalouser --target <threadId> --message "Hello from Aren!"
```
...
```bash
aren channels login --channel zalouser              # Login via QR
aren channels login --channel zalouser --account work
aren channels status --probe
aren channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
aren directory self --channel zalouser
aren directory peers list --channel zalouser --query "name"
aren directory groups list --channel zalouser --query "work"
aren directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
aren message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
aren message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `aren channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
aren channels login --channel zalouser --account work
aren message send --channel zalouser --account work --target <id> --message "Hello"
