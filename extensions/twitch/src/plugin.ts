/**
 * Twitch channel plugin for Aren.
 *
...
import { buildChannelConfigSchema } from "aren/plugin-sdk";
import type {
 ...
 * for Aren. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ArenConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ArenConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ArenConfig): boolean => {
