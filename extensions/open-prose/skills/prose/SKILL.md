...
metadata: { "aren": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Aren `sessions_spawn`
- **File I/O** == Aren `read`/`write`
- **Remote fetch** == Aren `web_fetch` (or `exec` with curl when POST is required)
