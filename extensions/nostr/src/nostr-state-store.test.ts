import type { PluginRuntime } from "aren/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AREN_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-nostr-"));
  process.env.AREN_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AREN_STATE_DIR = previous;
    }
