...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["arenPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["aren", "pollId"]) ??
    readNestedString(value, ["aren", "poll", "id"]) ??
    readNestedString(value, ["data", "arenPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "aren", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { arenPollId: pollId, pollId },
        },
      },
    ],
  };
