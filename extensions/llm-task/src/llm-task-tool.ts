...
// NOTE: This extension is intended to be bundled with Aren.
// When running from source (tests/dev), Aren internals live under src/.
...
export function createLlmTaskTool(api: ArenPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via aren.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-llm-task-"));
