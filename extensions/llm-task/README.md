...
`aren.invoke --each`) without adding new Aren code per workflow.
...
This extension depends on Aren internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Aren extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.aren/extensions` as a standalone plugin directory.
