...
import type { ArenPluginApi, ArenPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "aren-lobster-plugin-") {
...
function fakeApi(overrides: Partial<ArenPluginApi> = {}): ArenPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<ArenPluginToolContext> = {}): ArenPluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ArenPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
