...
function resolveArenRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AREN_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AREN_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveArenRoot();
