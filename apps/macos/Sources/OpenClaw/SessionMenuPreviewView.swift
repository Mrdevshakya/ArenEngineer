...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.aren", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ArenSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ArenSessionPreviewEntry,
    ...
    private static func cache(payload: ArenSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ArenChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ArenKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ArenKit.AnyCodable]) -> [ArenChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ArenChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ArenChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ArenChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ArenChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ArenChatMessage) -> String? {
