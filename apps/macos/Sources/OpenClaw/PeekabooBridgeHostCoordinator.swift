...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.aren", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ArenPeekabooBridgeServices?
    private static var arenSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Aren", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ArenPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.arenSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.arenSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ArenPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.aren.peekaboo")
