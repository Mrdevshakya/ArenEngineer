...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ArenOAuthStore.AnthropicOAuthStatus = ArenOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ArenOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ArenOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ArenOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ArenOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Aren can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ArenOAuthStore.AnthropicOAuthStatus,
