...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Aren token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: ArenOAuthStore.AnthropicOAuthStatus = ArenOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.aren", category: "anthropic-oauth")
...
enum ArenOAuthStore {
    ...
    private static let arenOAuthDirEnv = "AREN_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Aren OAuth token file not found"
            case .unreadableFile: "Aren OAuth token file not readable"
            case .invalidJSON: "Aren OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Aren OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Aren OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.arenOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".aren", isDirectory: true)
