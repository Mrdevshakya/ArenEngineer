...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ArenProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.aren", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ArenKit.AnyCodable]()) {
                $0[$1.key] = ArenKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ArenProtocol.AnyCodable?) -> [String: ArenProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ArenProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ArenKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ArenProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("aren.control.heartbeat")
    static let controlAgentEvent = Notification.Name("aren.control.agent")
}
