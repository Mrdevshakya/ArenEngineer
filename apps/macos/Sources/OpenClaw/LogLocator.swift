...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AREN_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/aren")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("aren-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("aren-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/aren/ (rolling or stdout), or nil if none exist.
