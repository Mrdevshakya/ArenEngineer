...
enum ArenLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ArenOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ArenFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ArenLogging.bootstrapIfNeeded()
        let label = ArenLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ArenOSLogHandler: LogHandler {
...
struct ArenFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ArenLogging.parseLabel(self.label)
