...
enum CommandResolver {
    private static let projectRootDefaultsKey = "aren.gatewayProjectRootPath"
    private static let helperName = "aren"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let arenEntry = root.appendingPathComponent("aren.mjs").path
        if FileManager().isReadableFile(atPath: arenEntry) { return arenEntry }
        let binEntry = root.appendingPathComponent("bin/aren.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/aren")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let arenPaths = self.arenManagedPaths(home: home)
        if !arenPaths.isEmpty {
            extras.insert(contentsOf: arenPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + arenPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func arenManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".aren"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func arenExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectArenExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("aren.mjs").path,
            root.appendingPathComponent("bin/aren.js").path,
        ]
    ...
    static func hasAnyArenInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.arenExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let arenPath = self.projectArenExecutable(projectRoot: root) {
                return [arenPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "aren", subcommand] + extraArgs
            }
            if let arenPath = self.arenExecutable(searchPaths: searchPaths) {
                return [arenPath, subcommand] + extraArgs
            }

            let missingEntry = """
            aren entrypoint missing (looked for dist/index.js or aren.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real aren CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/aren"
        ...
        \(cliSection)
        \(projectSection)
        if command -v aren >/dev/null 2>&1; then
          CLI="$(command -v aren)"
          aren \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/aren.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/aren.mjs"
            node "$PRJ/aren.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/aren.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/aren.js"
            node "$PRJ/bin/aren.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent aren"
          pnpm --silent aren \(quotedArgs);
        else
          echo "aren CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ArenConfigFile.loadDict()
