...
struct ConnectOptions {
    ...
    var clientId: String = "aren-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          aren-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: aren-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Aren macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Aren macOS Gateway Connect")
