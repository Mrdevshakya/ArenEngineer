...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.aren/aren.json. Prefer AREN_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.aren/aren.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.aren/aren.json. Restart the gateway to apply."
        }
