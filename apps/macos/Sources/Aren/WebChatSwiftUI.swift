...
private let webChatSwiftLogger = Logger(subsystem: "ai.aren", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ArenChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ArenChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ArenChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ArenChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ArenGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ArenChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ArenChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ArenChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ArenChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ArenChatView>) -> NSViewController
    {
