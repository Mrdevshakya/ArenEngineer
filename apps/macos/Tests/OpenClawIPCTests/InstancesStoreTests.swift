...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ArenProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ArenProtocol.AnyCodable] = [
            "presence": .init([ArenProtocol.AnyCodable(entry)]),
        ]
        let payload = ArenProtocol.AnyCodable(payloadMap)
