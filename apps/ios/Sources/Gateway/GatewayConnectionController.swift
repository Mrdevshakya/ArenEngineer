...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ArenCapability.canvas.rawValue, ArenCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ArenCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ArenCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ArenLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ArenCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ArenSystemCommand.notify.rawValue,
            ArenSystemCommand.which.rawValue,
            ArenSystemCommand.run.rawValue,
            ArenSystemCommand.execApprovalsGet.rawValue,
            ArenSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(ArenCapability.camera.rawValue) {
            commands.append(ArenCameraCommand.list.rawValue)
            commands.append(ArenCameraCommand.snap.rawValue)
            commands.append(ArenCameraCommand.clip.rawValue)
        }
        if caps.contains(ArenCapability.location.rawValue) {
            commands.append(ArenLocationCommand.get.rawValue)
        }
