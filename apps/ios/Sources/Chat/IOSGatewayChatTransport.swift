...
struct IOSGatewayChatTransport: ArenChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ArenChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ArenChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ArenChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ArenChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ArenChatAttachmentPayload]) async throws -> ArenChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ArenChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ArenChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ArenGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ArenGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ArenChatEventPayload.self)
                        {
