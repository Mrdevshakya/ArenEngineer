...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ArenCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ArenCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ArenCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ArenCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ArenCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__aren__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: ArenLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for aren:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case ArenLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case ArenCanvasCommand.present.rawValue,
                 ArenCanvasCommand.hide.rawValue,
                 ArenCanvasCommand.navigate.rawValue,
                 ArenCanvasCommand.evalJS.rawValue,
                 ArenCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case ArenCanvasA2UICommand.reset.rawValue,
                 ArenCanvasA2UICommand.push.rawValue,
                 ArenCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case ArenCameraCommand.list.rawValue,
                 ArenCameraCommand.snap.rawValue,
                 ArenCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case ArenScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ArenNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ArenNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ArenLocationGetParams.self, from: req.paramsJSON)) ??
            ArenLocationGetParams()
        ...
        let payload = ArenLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ArenCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(ArenCanvasPresentParams.self, from: req.paramsJSON)) ??
                ArenCanvasPresentParams()
        ...
        case ArenCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ArenCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ArenCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ArenCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ArenCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArenCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ArenCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ArenCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing arenA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ArenCanvasA2UICommand.push.rawValue, ArenCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == ArenCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ArenCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ArenCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ArenCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ArenCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.arenA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ArenCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArenCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ArenCameraSnapParams.self, from: req.paramsJSON)) ??
                ArenCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ArenCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ArenCameraClipParams.self, from: req.paramsJSON)) ??
                ArenCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ArenScreenRecordParams.self, from: req.paramsJSON)) ??
            ArenScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> ArenLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ArenLocationMode(rawValue: raw) ?? .off
    }
