...
private let chatUILogger = Logger(subsystem: "ai.aren", category: "ArenChatUI")

@MainActor
@Observable
public final class ArenChatViewModel {
    public private(set) var messages: [ArenChatMessage] = []
    ...
    public var attachments: [ArenPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ArenChatPendingToolCall] = []
    public private(set) var sessions: [ArenChatSessionEntry] = []
    private let transport: any ArenChatTransport
    ...
    private var pendingToolCallsById: [String: ArenChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ArenChatTransport) {
    ...
    public var sessionChoices: [ArenChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ArenChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ArenChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ArenPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ArenChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ArenChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ArenChatMessage]) -> [ArenChatMessage] {
        var result: [ArenChatMessage] = []
    ...
    private static func dedupeKey(for message: ArenChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ArenChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ArenChatAttachmentPayload in
            ArenChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ArenChatSessionEntry {
        ArenChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ArenChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ArenChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ArenAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ArenChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ArenPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
