...
@MainActor
struct ArenChatComposer: View {
    @Bindable var viewModel: ArenChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ArenChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ArenChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ArenPendingAttachment.id)
                { (att: ArenPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ArenPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ArenChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ArenChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ArenChatTheme.subtleCard)
        .clipShape(Capsule())
    }
