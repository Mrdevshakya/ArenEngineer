...
struct ChatMessageBubble: View {
    let message: ArenChatMessage
    let style: ArenChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ArenChatTheme.userText : ArenChatTheme.assistantText
    ...
    private var inlineAttachments: [ArenChatMessageContent] {
    ...
    private var toolCalls: [ArenChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ArenChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ArenChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ArenChatTheme.onboardingAssistantBubble
        }
        return ArenChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ArenChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ArenChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ArenChatTheme.userText : ArenChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ArenChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ArenChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ArenChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ArenChatTheme.assistantText)
            }
        }
    }
}
