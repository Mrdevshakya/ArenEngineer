...
private final class TestChatTransport: @unchecked Sendable, ArenChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ArenChatHistoryPayload]
    private let sessionsResponses: [ArenChatSessionsListResponse]

    private let stream: AsyncStream<ArenChatTransportEvent>
    private let continuation: AsyncStream<ArenChatTransportEvent>.Continuation

    init(
        historyResponses: [ArenChatHistoryPayload],
        sessionsResponses: [ArenChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ArenChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ArenChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ArenChatHistoryPayload(
        ...
        attachments _: [ArenChatAttachmentPayload]) async throws -> ArenChatSendResponse
    {
        ...
        return ArenChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ArenChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ArenChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ArenChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ArenChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ArenChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ArenChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ArenChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ArenChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ArenChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ArenChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
