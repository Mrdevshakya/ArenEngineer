---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Aren deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Aren cron scheduling for periodic checks, or version status checks on a machine running Aren (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Aren, then align it to a user-defined risk tolerance without breaking access. Use Aren security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Aren changes the host firewall, SSH, or OS updates; it does not.
...
5. Aren gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Aren can access sensitive data.
...
### 2) Run Aren security audits (read-only)

As part of the default read-only checks, run `aren security audit --deep`. Only offer alternatives if the user requests them:

1. `aren security audit` (faster, non-probing)
2. `aren security audit --json` (structured output)

Offer to apply Aren safe defaults (numbered):

1. `aren security audit --fix`

Be explicit that `--fix` only tightens Aren defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Aren version/update status (read-only)

As part of the default read-only checks, run `aren update status`.
...
- Credential hygiene notes (location of Aren creds, prefer disk encryption)
...
After Aren install or first hardening pass, run at least one baseline audit and version check:

- `aren security audit`
- `aren security audit --deep`
- `aren update status`

Ongoing monitoring is recommended. Use the Aren cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `aren cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `aren update status`

Also offer a periodic version check so the user can decide when to update (numbered):

1. `aren update status` (preferred for source checkouts and channels)
2. `npm view aren version` (published npm version)

## Aren command accuracy

Use only supported commands and flags:

- `aren security audit [--deep] [--fix] [--json]`
- `aren status` / `aren status --deep`
- `aren health --json`
- `aren update status`
- `aren cron add|list|runs|run`

Do not invent CLI flags or imply Aren enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Aren compaction:
