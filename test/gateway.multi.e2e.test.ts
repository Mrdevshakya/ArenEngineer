...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `aren-e2e-${name}-`));
  const configDir = path.join(homeDir, ".aren");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AREN_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AREN_GATEWAY_TOKEN: inst.gatewayToken,
        AREN_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AREN_GATEWAY_PORT: String(gwA.port),
          AREN_GATEWAY_TOKEN: gwA.gatewayToken,
          AREN_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AREN_GATEWAY_PORT: String(gwB.port),
          AREN_GATEWAY_TOKEN: gwB.gatewayToken,
          AREN_GATEWAY_PASSWORD: "",
        }),
