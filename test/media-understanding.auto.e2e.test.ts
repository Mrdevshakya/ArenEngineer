...
import type { ArenConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("aren-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("aren-bin-sherpa-");
      const modelDir = await makeTempDir("aren-sherpa-model-");
      ...
      const cfg: ArenConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aren-bin-whispercpp-");
      const modelDir = await makeTempDir("aren-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aren-bin-gemini-");
      ...
      const cfg: ArenConfig = { tools: { media: { image: {} } } };
