...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AREN_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.AREN_STATE_DIR = path.join(base, ".aren");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "aren-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".aren", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
