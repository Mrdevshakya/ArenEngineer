...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aren/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aren");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aren/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AREN_CONTROL_UI_BASE_PATH__ = "/aren";
    const app = mountApp("/aren/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aren");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aren/sessions");
  });
