...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as ArenApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as ArenApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as ArenApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as ArenApp);
    await loadDevices(host as unknown as ArenApp);
    await loadConfig(host as unknown as ArenApp);
    await loadExecApprovals(host as unknown as ArenApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as ArenApp);
    await loadConfig(host as unknown as ArenApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as ArenApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as ArenApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__AREN_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArenApp, false),
    loadPresence(host as unknown as ArenApp),
    loadSessions(host as unknown as ArenApp),
    loadCronStatus(host as unknown as ArenApp),
    loadDebug(host as unknown as ArenApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArenApp, true),
    loadConfigSchema(host as unknown as ArenApp),
    loadConfig(host as unknown as ArenApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ArenApp, false),
    loadCronStatus(host as unknown as ArenApp),
    loadCronJobs(host as unknown as ArenApp),
  ]);
}
