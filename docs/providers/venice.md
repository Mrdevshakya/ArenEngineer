---
summary: "Use Venice AI privacy-focused models in Aren"
read_when:
  - You want privacy-focused inference in Aren
...
```bash
aren onboard --auth-choice venice-api-key
```
...
```bash
aren chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Aren shows all available Venice models. Pick based on your needs:
...
```bash
aren models set venice/claude-opus-45
aren models set venice/llama-3.3-70b
```

List all available models:

```bash
aren models list | grep venice
```

## Configure via `aren configure`

1. Run `aren configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Aren automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
aren chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
aren chat --model venice/claude-opus-45

# Use uncensored model
aren chat --model venice/venice-uncensored

# Use vision model with image
aren chat --model venice/qwen3-vl-235b-a22b

# Use coding model
aren chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `aren models list` to see currently available models. Some models may be temporarily offline.
