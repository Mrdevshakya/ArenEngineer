---
summary: "Use Synthetic's Anthropic-compatible API in Aren"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Aren registers it as the
...
```bash
aren onboard --auth-choice synthetic-api-key
```
...
Note: Aren's Anthropic client appends `/v1` to the base URL, so use
