---
summary: "Use MiniMax M2.1 in Aren"
read_when:
  - You want MiniMax models in Aren
...
```bash
aren plugins enable minimax-portal-auth  # skip if already loaded.
aren gateway restart  # restart if gateway is already running
aren onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/aren/aren/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `aren.json`:
...
- Use `aren models list` and `aren models set minimax/MiniMax-M2.1` to switch.
...
- Running `aren configure` and selecting **MiniMax M2.1**, or
