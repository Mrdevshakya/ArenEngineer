...
- **gateway host** → `aren` process on the gateway machine
...
`~/.aren/exec-approvals.json` directly.

CLI: `aren approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
