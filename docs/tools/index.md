---
summary: "Agent tool surface for Aren (browser, canvas, nodes, message, cron) replacing legacy `aren-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `aren-*` skills
title: "Tools"
---

# Tools (Aren)

Aren exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `aren-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `aren.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Aren logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`aren node run`).
...
- Requires a Brave API key (recommended: `aren configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Aren-managed browser.
...
- Quick smoke: `aren nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `aren gateway` restart in-place)
