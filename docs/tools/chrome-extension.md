---
summary: "Chrome extension: let Aren drive your existing Chrome tab"
read_when:
...
The Aren Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate aren-managed Chrome profile.
...
Aren then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Aren release (npm package) as static files. There is no separate “build” step.

After upgrading Aren:

- Re-run `aren browser extension install` to refresh the installed files under your Aren state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Aren ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Aren to control.
...
- `ON`: attached; Aren can drive that tab.
...
Debugging: `aren sandbox explain`
...
`aren browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `aren browser extension install` first to copy the extension to a stable location under your Aren state directory.
...
- **This is not isolated** like the dedicated aren-managed profile.
