...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/aren`)
...
If the page fails to load, start the Gateway first: `aren gateway`.
...
you revoke it with `aren devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.aren/aren.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Aren
...
```bash
aren gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Aren **blocks** Control UI connections without device identity.
...
```bash
AREN_CONTROL_UI_BASE_PATH=/aren/ pnpm ui:build
```
