---
summary: "Research notes: offline memory system for Aren workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.aren/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Aren integration
...
Target: Aren-style workspace (`agents.defaults.workspace`, default `~/.aren/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.aren/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Aren to see every session.
...
```
~/.aren/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Aren**, but keep a separable core library.

### Why integrate into Aren?
...
- You want the agent itself to call the tools:
  - `aren memory recall "…" --k 25 --since 30d`
  - `aren memory reflect --since 7d`
...
Pragmatic take for `~/.aren/workspace`:
