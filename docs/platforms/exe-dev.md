---
summary: "Run Aren Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Aren Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`

This page assumes exe.dev's default **exeuntu** image. If you picked a different distro, map packages accordingly.

## Beginner quick path

1. [https://exe.new/aren](https://exe.new/aren)
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install Aren instantly with our
...
Tip: keep this VM **stateful**. Aren stores state under `~/.aren/` and `~/.aren/workspace/`.
...
## 5) Access Aren and grant privileges

Access `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL` (see the Control UI output from onboarding). Approve
devices with `aren devices list` and `aren devices approve <requestId>`. When in doubt,
