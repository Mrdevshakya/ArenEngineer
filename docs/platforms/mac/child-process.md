...
service via the external `aren` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`AREN_PROFILE`; legacy `com.aren.*` is supported).
