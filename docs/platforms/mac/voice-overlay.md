...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `bot.aren`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "bot.aren" AND category CONTAINS "voicewake"' --level info --style compact
  ```
