---
summary: "Aren on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Aren on a Raspberry Pi
  - Running Aren on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Aren on Raspberry Pi

## Goal

Run a persistent, always-on Aren Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Aren features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aren

# Enable if not
sudo systemctl enable aren

# Start on boot
sudo systemctl start aren
```
...
```bash
# Check logs
journalctl -u aren --no-pager -n 100

# Common fix: rebuild
cd ~/aren  # if using hackable install
npm run build
sudo systemctl restart aren
```
