...
- Choose install method:
  - `npm` (default): `npm install -g aren@latest`
...
- If upgrading an existing install: runs `aren doctor --non-interactive` (best effort).
- For git installs: runs `aren doctor --non-interactive` after install/update (best effort).
...
If you run the installer while **already inside a Aren source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `AREN_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `aren` into a prefix (default: `~/.aren`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g aren@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `aren doctor --non-interactive` on upgrades and git installs (best effort).
...
- **"aren" is not recognized**: your npm global bin folder is not on PATH. Most systems use
