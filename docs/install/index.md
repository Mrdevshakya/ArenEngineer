---
summary: "Install Aren (recommended installer, global install, or from source)"
read_when:
  - Installing Aren
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g aren@latest
```
...
pnpm approve-builds -g                # approve aren, node-llama-cpp, sharp, etc.
pnpm add -g aren@latest           # re-run to execute postinstall scripts
```
...
Tip: if you don’t have a global install yet, run repo commands via `pnpm aren ...`.
...
- Run onboarding: `aren onboard --install-daemon`
- Quick check: `aren doctor`
- Check gateway health: `aren status` + `aren health`
- Open the dashboard: `aren dashboard`
...
## Troubleshooting: `aren` not found (PATH)
...
If `$(npm prefix -g)/bin` (macOS/Linux) or `$(npm prefix -g)` (Windows) is **not** present inside `echo "$PATH"`, your shell can’t find global npm binaries (including `aren`).
