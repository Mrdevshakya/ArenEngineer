...
read_when:
  - "You installed Aren but `aren` is “command not found”"
...
Aren’s runtime baseline is **Node 22+**.

If you can run `npm install -g aren@latest` but later see `aren: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
