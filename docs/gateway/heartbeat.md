...
- During heartbeat runs, Aren treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aren sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Aren skips the heartbeat run entirely (no model call).
...
```bash
aren system event --text "Check for urgent follow-ups" --mode now
```
