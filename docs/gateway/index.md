...
- Replaces the legacy `gateway` command. CLI entry point: `aren gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
aren gateway --port 18789
...
- Config hot reload watches `~/.aren/aren.json` (or `AREN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__aren__/canvas/` from `~/.aren/workspace/canvas`. Disable with `canvasHost.enabled=false` or `AREN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `AREN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `AREN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `arent.<profile>` (legacy `bot.aren.*` may still exist)
- Linux: `aren-gateway-<profile>.service`
- Windows: `Aren Gateway (<profile>)`

Install metadata is embedded in the service config:

- `AREN_SERVICE_MARKER=aren`
- `AREN_SERVICE_KIND=gateway`
- `AREN_SERVICE_VERSION=<version>`
...
```bash
aren --dev setup
aren --dev gateway --allow-unconfigured
...
- `AREN_STATE_DIR=~/.aren-dev`
- `AREN_CONFIG_PATH=~/.aren-dev/aren.json`
- `AREN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.aren/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `AREN_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `AREN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `AREN_CONFIG_PATH`
- unique `AREN_STATE_DIR`
...
```bash
aren --profile main gateway install
aren --profile rescue gateway install
```

Example:

```bash
AREN_CONFIG_PATH=~/.aren/a.json AREN_STATE_DIR=~/.aren-a aren gateway --port 19001
AREN_CONFIG_PATH=~/.aren/b.json AREN_STATE_DIR=~/.aren-b aren gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `aren health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `aren`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `aren gateway install` writes `~/Library/LaunchAgents/arent.gateway.plist`
    (or `arent.<profile>.plist`; legacy `bot.aren.*` is cleaned up).
...
- If other gateway-like services are detected, the CLI warns unless they are Aren profile services.
  ...
  - Cleanup: `aren gateway uninstall` (current service) and `aren doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `aren gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Aren.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `arent.gateway` (or `arent.<profile>`; legacy `bot.aren.*` labels still unload cleanly).
- To stop it cleanly, use `aren gateway stop` (or `launchctl bootout gui/$UID/arent.gateway`).
- To restart, use `aren gateway restart` (or `launchctl kickstart -k gui/$UID/arent.gateway`).
...
Aren installs a **systemd user service** by default on Linux/WSL2. We
...
`aren gateway install` writes the user unit. `aren doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/aren-gateway[-<profile>].service`:

```
[Unit]
Description=Aren Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/aren gateway --port 18789
Restart=always
RestartSec=5
Environment=AREN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now aren-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/aren-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now aren-gateway[-<profile>].service
```
...
- `aren gateway health|status` — request health/status over the Gateway WS.
- `aren message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `aren agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `aren gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `aren gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `aren gateway` and the legacy TCP control port.
