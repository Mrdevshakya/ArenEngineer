---
summary: "Quick troubleshooting guide for common Aren failures"
read_when:
...
When Aren misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `aren status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `aren status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `aren status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `aren gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `aren channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `aren gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `aren logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `aren status --all` (it redacts tokens). If you paste `aren status`, consider setting `AREN_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  aren models auth setup-token --provider anthropic
  ```
...
```bash
aren models auth paste-token --provider anthropic
aren models status
```
...
- Preferred: `aren logs --follow`
- File logs (always): `/tmp/aren/aren-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$AREN_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u aren-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  aren config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  aren config set gateway.mode remote
  aren config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `aren setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.aren/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `aren doctor`
...
- then run `aren sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `aren gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `aren gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `aren gateway status` and `aren doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `AREN_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `aren gateway status` shows a config mismatch**
...
- Fix: rerun `aren gateway install --force` from the same `--profile` / `AREN_STATE_DIR` you want the service to use.

**If `aren gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `aren doctor` to update it (or `aren gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `AREN_GATEWAY_TOKEN`) and restart the service.

**If `aren gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/aren` on disk.
...
Symptoms: `pwd` or file tools show `~/.aren/sandboxes/...` even though you
...
Aren intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `aren models list` or
  `aren models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${AREN_CONFIG_PATH:-$HOME/.aren/aren.json}"
```
...
tail -f "$(ls -t /tmp/aren/aren-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
aren logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.aren/agents/<agentId>/sessions/
```
...
aren logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
aren gateway --verbose
```

If you’re logged out / unlinked:

```bash
aren channels logout
trash "${AREN_STATE_DIR:-$HOME/.aren}/credentials" # if logout can't cleanly remove everything
aren channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/aren/aren-*.log | head -1)" | tail -20
```

### High Memory Usage

Aren keeps conversation history in memory.
...
Aren now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `aren doctor` reports every invalid entry.
- `aren doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `aren logs`, `aren health`, `aren status`, `aren gateway status`, and `aren gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/aren/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `aren models status` (CLI).
...
5. Run `aren channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Aren scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/aren/aren/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
Ensure the global `aren` CLI is installed and matches the app version:

```bash
aren --version
npm install -g aren@<version>
```
...
#   ${AREN_CONFIG_PATH:-$HOME/.aren/aren.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
aren gateway --verbose
aren channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/aren/aren-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$AREN_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.aren/logs/...`; profiles use `~/.aren-<profile>/logs/...`)<br />Linux: `journalctl --user -u aren-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Aren Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$AREN_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$AREN_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$AREN_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
trash "${AREN_STATE_DIR:-$HOME/.aren}"
aren channels login         # re-pair WhatsApp
aren gateway restart           # or: aren gateway
```
...
1. Check logs first: `/tmp/aren/` (default: `aren-YYYY-MM-DD.log`, or your configured `logging.file`)
