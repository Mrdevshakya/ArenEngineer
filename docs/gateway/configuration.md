---
summary: "All configuration options for ~/.aren/aren.json with examples"
read_when:
...
Aren reads an optional **JSON5** config from `~/.aren/aren.json` (comments + trailing commas allowed).

If the file is missing, Aren uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.aren/workspace`). You usually only need a config to:
...
Aren only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `aren doctor`, `aren logs`, `aren health`, `aren status`, `aren service`, `aren help`).
- Run `aren doctor` to see the exact issues.
- Run `aren doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `aren config set`. Keep a backup of `~/.aren/aren.json`.
...
```bash
aren gateway call config.get --params '{}' # capture payload.hash
aren gateway call config.apply --params '{
...
```bash
aren gateway call config.get --params '{}' # capture payload.hash
aren gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.aren/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@aren", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.aren/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.aren/workspace" }],
}
```
...
{ "$include": "/etc/aren/base.json5" } // absolute
...
```json5
// ~/.aren/clients/mueller/agents.json5
[
...
```json5
// ~/.aren/clients/mueller/broadcast.json5
{
...
Aren reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Aren runs your login shell and imports only the missing expected keys (never overrides).
...
- `AREN_LOAD_SHELL_ENV=1`
- `AREN_SHELL_ENV_TIMEOUT_MS=15000`
...
Aren stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.aren/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.aren/credentials/oauth.json` (or `$AREN_STATE_DIR/credentials/oauth.json`)
...
- `~/.aren/agent/*` (migrated by `aren doctor` into `~/.aren/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `AREN_OAUTH_DIR`
- Agent dir (default agent root override): `AREN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Aren imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Aren derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/aren/aren-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/aren/aren.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.aren/credentials/whatsapp/biz
          // authDir: "~/.aren/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `aren doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@aren", "aren"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@aren"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.aren/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.aren/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.aren/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.aren/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.aren/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.aren/workspace-home" },
      { id: "work", workspace: "~/.aren/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `aren.json`).
...
Aren starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["aren-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-aren",
...
Aren starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `aren plugins install @aren/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Aren starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Aren spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Aren
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/aren" } },
}
```
...
When a file exceeds this limit, Aren logs a warning and injects a truncated
...
message envelopes). If unset, Aren uses the host timezone at runtime.
...
`[aren]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[aren]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[aren]"`, Aren will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Aren generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.aren/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Aren"`).
...
Aren also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Aren currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `aren configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:aren`: all built-in Aren tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.aren/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.aren/sandboxes",
        docker: {
          image: "aren-sandbox:bookworm-slim",
          containerPrefix: "aren-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "aren-sandbox-browser:bookworm-slim",
          containerPrefix: "aren-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Aren uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.aren/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Aren config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Aren writes/merges a `models.json` into
`~/.aren/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Aren uses
...
- Shortcut: `aren onboard --auth-choice opencode-zen`.
...
Shortcut: `aren onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `aren onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `aren onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `aren onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `aren onboard --auth-choice minimax-api`.
...
- Override the agent config root with `AREN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.aren/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.aren/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.aren/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Aren stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.aren/skills` (workspace skills
...
from `~/.aren/extensions`, `<workspace>/.aren/extensions`, plus any
...
### `browser` (aren-managed browser)

Aren can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for aren and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Aren.app menubar, or `aren gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      aren: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/aren" }
...
- When a connection comes from one of these IPs, Aren uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `aren gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `AREN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `AREN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Aren
...
- Aren.app watches `~/.aren/aren.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.aren/aren.json` (or `AREN_CONFIG_PATH`) and applies changes automatically.
...
- `~/.aren/aren.json` (or `AREN_CONFIG_PATH`)
...
- `AREN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `aren --dev …` → uses `~/.aren-dev` + shifts ports from base `19001`
- `aren --profile <name> …` → uses `~/.aren-<name>` (port via config/env/flags)
...
```bash
AREN_CONFIG_PATH=~/.aren/a.json \
AREN_STATE_DIR=~/.aren-a \
aren gateway --port 19001
```
...
Gmail helper config (used by `aren webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "aren@gmail.com",
...
- Set `AREN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Aren defaults `serve.path` to `/` so
...
Default root: `~/.aren/workspace/canvas`  
Default port: `18793` (chosen to avoid the aren browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__aren__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__aren__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.aren/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.aren/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.aren/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_aren-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `aren` (advertises `aren.local`). Override with `AREN_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_aren-gw._tcp` under `~/.aren/dns/` using the configured discovery domain (example: `aren.internal.`).
