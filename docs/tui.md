...
```bash
aren tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `aren tui --deliver`
...
- Check the Gateway logs: `aren logs --follow`.
- Confirm the agent can run: `aren status` and `aren models status`.
...
- No agents in picker: check `aren agents list` and your routing config.
