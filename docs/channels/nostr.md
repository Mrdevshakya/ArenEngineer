...
read_when:
  - You want Aren to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Aren to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`aren onboard`) and `aren channels add` list optional channel plugins.
...
```bash
aren plugins install @aren/nostr
```

Use a local checkout (dev workflows):

```bash
aren plugins install --link <path-to-aren>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "aren@example.com",
        "lud16": "aren@example.com"
      }
    }
  }
}
