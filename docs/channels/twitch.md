...
read_when:
  - Setting up Twitch chat integration for Aren
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Aren connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
aren plugins install @aren/twitch
```

Local checkout (when running from a git repo):

```bash
aren plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `AREN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "aren", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AREN_TWITCH_ACCESS_TOKEN env var)
...
```bash
AREN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
