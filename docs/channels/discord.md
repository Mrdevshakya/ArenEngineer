...
- Talk to Aren via Discord DMs or guild channels.
...
3. Configure Aren with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `aren pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Aren in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Aren needs
...
Discord uses numeric ids everywhere; Aren config prefers ids.
...
- First: run `aren doctor` and `aren channels status --probe` (actionable warnings + quick audits).
...
When enabled, Aren uses the member identity for allowlists and labels the
...
- On startup, Aren resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror Aren’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Aren enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`aren gateway --force`) after confirming no other processes own the Discord session.
