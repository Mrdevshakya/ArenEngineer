...
```bash
aren plugins install @aren/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
aren plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Aren" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `aren pairing list nextcloud-talk`
  - `aren pairing approve nextcloud-talk <CODE>`
