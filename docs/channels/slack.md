...
3. Set tokens for Aren and start the gateway.
...
7. Slash Commands → create `/aren` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Aren can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Aren falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Aren",
    "description": "Slack connector for Aren"
  },
  "features": {
    "bot_user": {
      "display_name": "Aren",
    ...
    "slash_commands": [
      {
        "command": "/aren",
        "description": "Send a message to Aren",
        "should_escape": false
      }
    ]
  },
...
By default, Aren replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Aren infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `aren pairing approve slack <code>`.
...
- On startup, Aren resolves channel/user names in allowlists to IDs (when tokens allow)
