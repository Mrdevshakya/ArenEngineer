...
Matrix is an open, decentralized messaging protocol. Aren connects as a Matrix **user**
...
```bash
aren plugins install @aren/matrix
```

Local checkout (when running from a git repo):

```bash
aren plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `aren plugins install @aren/matrix`
   - From a local checkout: `aren plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Aren calls the same
     login endpoint, stores the access token in `~/.aren/credentials/matrix/credentials.json`,
...
- On first connection, Aren requests device verification from your other sessions.
...
`~/.aren/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `aren pairing list matrix`
  - `aren pairing approve matrix <CODE>`
...
- On startup, Aren resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
