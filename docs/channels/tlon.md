...
Tlon is a decentralized messenger built on Urbit. Aren connects to your Urbit ship and can
...
```bash
aren plugins install @aren/tlon
```

Local checkout (when running from a git repo):

```bash
aren plugins install ./extensions/tlon
```
...
Use these with `aren message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Aren replies in-thread.
