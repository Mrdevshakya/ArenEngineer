...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Aren
...
Running Aren via Node with `tsx` fails at startup with:

```
[aren] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `aren status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch aren.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node aren.mjs status` works on Node 25.
