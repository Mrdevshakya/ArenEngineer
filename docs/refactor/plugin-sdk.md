...
- Publish as `aren/plugin-sdk` (or export from core under `aren/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ArenPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ArenConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ArenConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ArenConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ArenConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ArenConfig): string;
  };
};
```
...
- Introduce `aren/plugin-sdk`.
- Add `api.runtime` to `ArenPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `arenRuntime: ">=2026.2.0"`).
