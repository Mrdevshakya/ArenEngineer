---
summary: "First-run onboarding flow for Aren (macOS app)"
read_when:
...
- Writes credentials to `~/.aren/credentials/oauth.json`
...
The app can install the global `aren` CLI via npm/pnpm so terminal
...
On the first agent run, Aren bootstraps a workspace (default `~/.aren/workspace`):
...
```bash
aren webhooks gmail setup --account you@gmail.com
```
