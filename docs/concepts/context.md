...
read_when:
  - You want to understand what “context” means in Aren
...
“Context” is **everything Aren sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Aren-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Aren builds the system prompt

The system prompt is **Aren-owned** and rebuilt each run. It includes:
...
By default, Aren injects a fixed set of workspace files (if present):
