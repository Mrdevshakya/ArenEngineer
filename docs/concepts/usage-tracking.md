...
- `/usage cost` in chats: local cost summary aggregated from Aren session logs.
- CLI: `aren status --usage` prints a full per-provider breakdown.
- CLI: `aren channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
