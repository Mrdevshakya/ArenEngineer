...
Deploy Aren on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/aren/aren"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run Aren:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AREN_STATE_DIR
        value: /data/.aren
      - key: AREN_WORKSPACE_DIR
        value: /data/workspace
      - key: AREN_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/aren`.
...
If you use the original Aren repository, Render will not auto-deploy your Aren. To update it, run a manual Blueprint sync from the dashboard.
...
For Aren, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Aren host.
