...
Deploy Aren on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/aren-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/aren` — Control UI

## What you get

- Hosted Aren Gateway + Control UI
...
- `AREN_STATE_DIR=/data/.aren` (recommended)
- `AREN_WORKSPACE_DIR=/data/workspace` (recommended)
- `AREN_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Aren state + workspace so you can migrate to another host without losing config or memory.
