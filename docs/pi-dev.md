...
This guide summarizes a sane workflow for working on the pi integration in Aren.
...
- Trigger the agent directly:
  - `pnpm aren agent --message "Hello" --thinking low`
...
State lives under the Aren state directory. Default is `~/.aren`. If `AREN_STATE_DIR` is set, use that directory instead.

To reset everything:

- `aren.json` for config
