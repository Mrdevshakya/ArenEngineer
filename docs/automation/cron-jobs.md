...
- Jobs persist under `~/.aren/cron/` so restarts don’t lose schedules.
...
aren cron list
aren cron run <job-id> --force
aren cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.aren/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `aren cron add/edit` or the cron
...
- Job store: `~/.aren/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.aren/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
aren cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
aren cron edit <jobId> --agent ops
aren cron edit <jobId> --clear-agent
```
...
```bash
aren system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`aren system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `AREN_SKIP_CRON`.
