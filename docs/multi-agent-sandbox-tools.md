...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `aren sandbox explain`.
...
Legacy `agent.*` configs are migrated by `aren doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=aren-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${AREN_STATE_DIR:-$HOME/.aren}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
