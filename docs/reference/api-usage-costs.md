...
Aren features that can generate provider usage or paid API calls.
...
- `aren status --usage` and `aren channels list` show provider **usage windows**
...
Aren can pick up credentials from:
...
`aren models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
