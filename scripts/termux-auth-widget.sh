...
# Check auth status
termux-toast "Checking Aren auth..."

STATUS=$(ssh "$SERVER" '$HOME/aren/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/aren/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AREN_EXPIRING|AREN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/aren/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AREN_EXPIRED|AREN_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Aren won't work until re-auth"
                ;;
        esac
        ;;
