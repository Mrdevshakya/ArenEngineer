...
#
# Suggested cron: */30 * * * * /home/admin/aren/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Aren notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aren-alerts)
...
STATE_FILE="$HOME/.aren/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Aren if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aren
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Aren to $NOTIFY_PHONE..."
            aren send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Aren is down. Run: ssh l36 '~/aren/scripts/mobile-reauth.sh'" "urgent"
    exit 1
