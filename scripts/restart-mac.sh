#!/usr/bin/env bash
# Reset Aren like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${AREN_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Aren.app/Contents/MacOS/Aren"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Aren"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Aren"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Aren"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.aren.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/aren-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${AREN_RESTART_LOG:-/tmp/aren-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${AREN_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.aren/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  AREN_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node aren.mjs daemon install --force --runtime node"
      log "  node aren.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.aren/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.aren.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Aren instances"
kill_all_aren
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Aren"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Aren.app" ]]; then
    APP_BUNDLE="/Applications/Aren.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Aren.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Aren.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Aren.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set AREN_APP_BUNDLE to your installed Aren.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aren.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aren.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".aren", "aren.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Aren is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/bot.aren.gateway.plist' | head -n 40 || true"
fi
