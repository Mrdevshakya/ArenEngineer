...
IMAGE_NAME="aren-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="aren-net-e2e-$$"
GW_NAME="aren-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AREN_GATEWAY_TOKEN=$TOKEN" \
	  -e "AREN_SKIP_CHANNELS=1" \
	  -e "AREN_SKIP_GMAIL_WATCHER=1" \
	  -e "AREN_SKIP_CRON=1" \
	  -e "AREN_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
