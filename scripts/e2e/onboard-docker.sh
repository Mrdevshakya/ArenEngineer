...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/aren-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/aren-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/aren-e2e-$1.XXXXXX"
  }
