...
BASE_IMAGE="${BASE_IMAGE:-aren-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aren-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aren.sandbox=1)
NOTE
