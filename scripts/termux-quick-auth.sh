...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/aren/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Aren Re-Auth" -c "After getting key, run: ssh $SERVER '~/aren/scripts/mobile-reauth.sh'" --id aren-auth
        ;;
    *)
