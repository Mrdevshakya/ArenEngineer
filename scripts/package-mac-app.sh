...
# Build and bundle Aren into a minimal .app we can open.
# Outputs to dist/Aren.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Aren.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Aren"
BUNDLE_ID="${BUNDLE_ID:-ai.aren.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/aren/aren/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Aren/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ArenBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ArenGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Aren"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Aren"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Aren"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Aren" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Aren/Resources/Aren.icns" "$APP_ROOT/Contents/Resources/Aren.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Aren/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ArenKit resources"
ARENKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ArenKit_ArenKit.bundle"
if [ -d "$ARENKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ArenKit_ArenKit.bundle"
  cp -R "$ARENKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ArenKit_ArenKit.bundle"
else
  echo "WARN: ArenKit resource bundle not found at $ARENKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Aren"
killall -q Aren 2>/dev/null || true
