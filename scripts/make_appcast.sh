...
ZIP=${1:?"Usage: $0 Aren-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/aren/aren/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Aren-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/aren/aren/releases/download/v${VERSION}/"}
