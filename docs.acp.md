...
This document describes how the Aren ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`aren acp` exposes an ACP agent over stdio and forwards prompts to a running
Aren Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Aren Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `aren acp` over stdio.

Example config:

```bash
aren config set gateway.remote.url wss://gateway-host:18789
aren config set gateway.remote.token <token>
```

Example run:

```bash
aren acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
aren acp --session agent:main:main
aren acp --session agent:design:main
aren acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Aren ACP” to start a thread.

## Execution Model

- ACP client spawns `aren acp` and speaks ACP messages over stdio.
...
```bash
aren acp --session agent:main:main
aren acp --session-label "support inbox"
aren acp --reset-session
```
...
`aren acp` resolves the Gateway URL and auth from CLI flags or config:
