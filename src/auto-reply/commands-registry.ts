...
import type { ArenConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: ArenConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ArenConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ArenConfig),
...
export function getCommandDetection(_cfg?: ArenConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ArenConfig) {
