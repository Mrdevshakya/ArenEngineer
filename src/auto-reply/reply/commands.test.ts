...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-commands-"));
...
function buildParams(commandBody: string, cfg: ArenConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
