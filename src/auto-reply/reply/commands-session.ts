...
import { scheduleGatewaySigusr1Restart, triggerArenRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Aren in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerArenRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Aren via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
