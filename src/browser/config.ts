import type { BrowserConfig, BrowserProfileConfig, ArenConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AREN_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "aren" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AREN_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AREN_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Aren browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for aren/aren CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ArenConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AREN_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "aren";
