...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".aren-profile-decorated");
}
 ...
 */
export function decorateArenProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AREN_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AREN_BROWSER_COLOR).toUpperCase();
