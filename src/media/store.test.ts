...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AREN_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "aren-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AREN_STATE_DIR = path.join(home, ".aren");
    ...
    await fs.mkdir(path.join(home, ".aren"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.aren${path.sep}media`);
