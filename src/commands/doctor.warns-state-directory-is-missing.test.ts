...
beforeEach(() => {
  ...
  resolveArenPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AREN_STATE_DIR;
  originalUpdateInProgress = process.env.AREN_UPDATE_IN_PROGRESS;
  process.env.AREN_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aren-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AREN_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AREN_UPDATE_IN_PROGRESS;
  } else {
    process.env.AREN_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveArenPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadArenPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/aren-root.js", () => ({
  resolveArenPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "aren-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AREN_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AREN_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/aren.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AREN_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
