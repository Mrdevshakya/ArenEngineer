...
beforeAll(() => {
  previousProfile = process.env.AREN_PROFILE;
  process.env.AREN_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.AREN_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/aren-root.js", () => ({
  resolveArenPackageRoot: vi.fn().mockResolvedValue("/tmp/aren"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/aren/pnpm-lock.yaml",
      markerPath: "/tmp/aren/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.aren.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.aren.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Aren status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("aren status --all") ||
          l.includes("aren --profile isolated status --all") ||
          l.includes("aren status --all") ||
          l.includes("aren --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.AREN_GATEWAY_TOKEN = "abcd1234";
    try {
