...
export function listAgentEntries(cfg: ArenConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ArenConfig, agentId: string) {
...
function resolveAgentModel(cfg: ArenConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ArenConfig): AgentSummary[] {
