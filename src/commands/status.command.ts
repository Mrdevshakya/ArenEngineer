...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Aren status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("aren security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("aren security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("aren status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("aren logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("aren status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("aren gateway probe")}`);
  }
}
