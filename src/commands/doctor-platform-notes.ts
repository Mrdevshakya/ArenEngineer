...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".aren", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: ArenConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["AREN_GATEWAY_TOKEN", await getenv("AREN_GATEWAY_TOKEN")],
    ["AREN_GATEWAY_PASSWORD", await getenv("AREN_GATEWAY_PASSWORD")],
    ["AREN_GATEWAY_TOKEN", await getenv("AREN_GATEWAY_TOKEN")],
    ["AREN_GATEWAY_PASSWORD", await getenv("AREN_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`AREN_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "AREN_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(
      ([key, value]) =>
        (key.startsWith("AREN_") || key.startsWith("AREN_")) && value?.trim(),
    )
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  ...
  const lines = [
    ...
    "- Use AREN_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> AREN_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
