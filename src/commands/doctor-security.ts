...
import type { ArenConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ArenConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("aren security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("aren configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("aren config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("aren doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("aren config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
