...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveArenAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("aren models auth setup-token")}\` or \`${formatCliCommand("aren configure")}\`.`
          : `Run \`${formatCliCommand("aren configure")}\` or set an API key env var.`;
