...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveArenAgentDir: vi.fn().mockReturnValue("/tmp/aren-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/aren-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/aren-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveArenAgentDir: mocks.resolveArenAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveArenAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/aren-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/aren-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/aren-agent-custom");
