...
import { readConfigFileSnapshot, type ArenConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ArenConfig, model: string): ArenConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("aren plugins install")}\`.`,
    );
  }
