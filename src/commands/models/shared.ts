...
export async function updateConfig(
  mutator: (cfg: ArenConfig) => ArenConfig,
): Promise<ArenConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: ArenConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ArenConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("aren agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
