...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AREN_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AREN_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AREN_GATEWAY_TOKEN");
    expect(message).not.toContain("AREN_GATEWAY_PASSWORD");
  });
