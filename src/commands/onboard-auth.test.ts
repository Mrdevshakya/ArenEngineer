...
const requireAgentDir = () => {
  const agentDir = process.env.AREN_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AREN_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.AREN_STATE_DIR;
  const previousAgentDir = process.env.AREN_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.AREN_AGENT_DIR;
    } else {
      process.env.AREN_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.AREN_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AREN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-oauth-"));
    process.env.AREN_STATE_DIR = tempStateDir;
    process.env.AREN_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AREN_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AREN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-minimax-"));
    process.env.AREN_STATE_DIR = tempStateDir;
    process.env.AREN_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AREN_AGENT_DIR;
