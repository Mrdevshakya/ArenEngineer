...
import { ensureArenCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.AREN_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.AREN_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureArenCliOnPath({ pathEnv: process.env.PATH ?? "" });
