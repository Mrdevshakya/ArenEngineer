...
This directory contains hooks that ship with Aren. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.aren/aren.json`:
...
2. Restart gateway: `pkill -9 -f 'aren.*gateway' && pnpm aren gateway`
3. Enable the hook: `aren hooks enable my-hook`
