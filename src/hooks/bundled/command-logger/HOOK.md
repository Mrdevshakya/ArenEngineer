...
homepage: https://docs.aren.engineer/hooks#command-logger
metadata:
  {
    "aren":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.aren/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.aren/logs/commands.log ~/.aren/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/aren`:
   ```
   /home/username/.aren/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
