...
`;
    ...
    expect(parsed.aren.emoji).toBe("💾");
    expect(parsed.aren.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.aren.emoji).toBe("📝");
    expect(parsed.aren.events).toEqual(["command"]);
    expect(parsed.aren.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.aren.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"aren": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"aren": {"events": ["test"]}}');
  });
...
describe("resolveArenMetadata", () => {
  it("extracts aren metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveArenMetadata(frontmatter);
  ...
  it("returns undefined when aren key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        aren: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Aren" },
            { id: "npm", kind: "npm", package: "@aren/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@aren/hook");
  });
    ...
    "aren":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Aren" }],
      },
  }
---
...
`;
    ...
    const aren = resolveArenMetadata(frontmatter);
    expect(aren).toBeDefined();
    expect(aren?.emoji).toBe("💾");
    expect(aren?.events).toEqual(["command:new"]);
    expect(aren?.requires?.config).toEqual(["workspace.dir"]);
    expect(aren?.install?.[0].kind).toBe("bundled");
  });
