...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "aren.sandbox=1");
  args.push("--label", `aren.sessionKey=${params.scopeKey}`);
  args.push("--label", `aren.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `aren.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("aren.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`aren sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`aren sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("aren sandbox recreate --all");
}
