...
import { __testing, createArenCodingTools } from "./pi-tools.js";
...
describe("createArenCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createArenCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "aren-outside.txt");
