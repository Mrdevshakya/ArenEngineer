...
/**
 * Cleanup suspended Aren CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ArenConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-cli-images-"));
  const paths: string[] = [];
