...
const defaultTools = createArenCodingTools();

describe("createArenCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aren-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createArenCodingTools();
    ...
    try {
      ...
      const contents = "Hello from aren read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "aren-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "aren-workspace"),
    ...
    const tools = createArenCodingTools({ sandbox });
