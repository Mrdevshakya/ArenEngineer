...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    AREN_STATE_DIR: process.env.AREN_STATE_DIR,
    AREN_CONFIG_PATH: process.env.AREN_CONFIG_PATH,
    AREN_GATEWAY_PORT: process.env.AREN_GATEWAY_PORT,
    AREN_PROFILE: process.env.AREN_PROFILE,
  };

  beforeEach(() => {
    process.env.AREN_STATE_DIR = "/tmp/aren-cli-state";
    process.env.AREN_CONFIG_PATH = "/tmp/aren-cli-state/aren.json";
    delete process.env.AREN_GATEWAY_PORT;
    delete process.env.AREN_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.AREN_STATE_DIR !== undefined) {
      process.env.AREN_STATE_DIR = originalEnv.AREN_STATE_DIR;
    } else {
      delete process.env.AREN_STATE_DIR;
    }

    if (originalEnv.AREN_CONFIG_PATH !== undefined) {
      process.env.AREN_CONFIG_PATH = originalEnv.AREN_CONFIG_PATH;
    } else {
      delete process.env.AREN_CONFIG_PATH;
    }

    if (originalEnv.AREN_GATEWAY_PORT !== undefined) {
      process.env.AREN_GATEWAY_PORT = originalEnv.AREN_GATEWAY_PORT;
    } else {
      delete process.env.AREN_GATEWAY_PORT;
    }

    if (originalEnv.AREN_PROFILE !== undefined) {
      process.env.AREN_PROFILE = originalEnv.AREN_PROFILE;
    } else {
      delete process.env.AREN_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AREN_PROFILE: "dev",
        AREN_STATE_DIR: "/tmp/aren-daemon-state",
