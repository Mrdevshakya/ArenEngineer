...
import { resolveArenPackageRoot } from "../infra/aren-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "aren doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "Arening complete. Please don't look at my soft shell phase.",
  "Version bump! Same chaos energy, fewer crashes (probably).",
];

const MAX_LOG_CHARS = 8000;
const DEFAULT_PACKAGE_NAME = "aren";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const AREN_REPO_URL = "https://github.com/aren/aren.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".aren");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("aren@")) {
    return trimmed.slice("aren@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.AREN_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AREN_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AREN_GIT_DIR points at a non-git directory: ${params.dir}. Set AREN_GIT_DIR to an empty folder or an aren checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AREN_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Aren update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Aren..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Aren install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("aren doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("aren gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g aren@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g aren@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AREN_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AREN_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("aren gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aren doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("aren gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aren gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `aren update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AREN_GIT_DIR points at a non-git directory: ${gitDir}. Set AREN_GIT_DIR to an empty folder or an aren checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AREN_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Aren to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aren update", "Update a source checkout (git)"],
        ["aren update --channel beta", "Switch to beta channel (git + npm)"],
        ["aren update --channel dev", "Switch to dev channel (git + npm)"],
        ["aren update --tag beta", "One-off update to a dist-tag or version"],
        ["aren update --no-restart", "Update without restarting the service"],
        ["aren update --json", "Output result as JSON"],
        ["aren update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aren update wizard", "Interactive update wizard"],
        ["aren --update", "Shorthand for aren update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aren update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.aren.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aren update status", "Show channel + version status."],
          ["aren update status --json", "JSON output."],
          ["aren update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
