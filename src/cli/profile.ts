...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.aren${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.AREN_PROFILE = profile;

  const stateDir = env.AREN_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.AREN_STATE_DIR?.trim()) {
    env.AREN_STATE_DIR = stateDir;
  }

  if (!env.AREN_CONFIG_PATH?.trim()) {
    env.AREN_CONFIG_PATH = path.join(stateDir, "aren.json");
  }

  if (profile === "dev" && !env.AREN_GATEWAY_PORT?.trim()) {
    env.AREN_GATEWAY_PORT = "19001";
  }
}
