...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("aren node install"), formatCliCommand("aren node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("aren node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AREN_LOG_PREFIX: baseEnv.AREN_LOG_PREFIX ?? "node",
