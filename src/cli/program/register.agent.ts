...
${formatHelpExamples([
  ['aren agent --to +15555550123 --message "status update"', "Start a new session."],
  ['aren agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'aren agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'aren agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['aren agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.aren.engineer/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.aren.engineer/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['aren agents set-identity --agent main --name "Aren" --emoji "🦞"', "Set name + emoji."],
  ["aren agents set-identity --agent main --avatar avatars/aren.png", "Set avatar path."],
  [
    "aren agents set-identity --workspace ~/.aren/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "aren agents set-identity --identity-file ~/.aren/workspace/IDENTITY.md --agent main",
