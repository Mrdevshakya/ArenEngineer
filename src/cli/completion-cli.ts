...
export async function installCompletion(shell: string, yes: boolean, binName = "aren") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c aren -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'aren gateway' and not 'aren other gateway', we need to check the sequence.
