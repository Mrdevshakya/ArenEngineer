...
import { loadArenPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AREN_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aren-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "aren.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AREN_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadArenPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AREN_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadArenPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadArenPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
