...
import { discoverArenPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ArenPluginDefinition;
  register?: ArenPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ArenPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ArenPluginDefinition;
...
export function loadArenPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverArenPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "aren/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ArenPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ArenPluginModule;
    } catch (err) {
