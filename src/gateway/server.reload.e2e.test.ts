...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AREN_SKIP_CHANNELS;
    prevSkipGmail = process.env.AREN_SKIP_GMAIL_WATCHER;
    process.env.AREN_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AREN_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AREN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AREN_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
