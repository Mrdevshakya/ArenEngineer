...
function setSlackDmPolicy(cfg: ArenConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Aren";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Aren`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: ArenConfig, allowFrom: string[]): ArenConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
