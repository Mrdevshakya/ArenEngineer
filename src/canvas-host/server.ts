...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.arenCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.arenCanvasA2UIAction &&
        typeof window.arenCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.arenSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("aren:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Aren node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.arenSendUserAction === "function"
        ? window.arenSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AREN_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AREN_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".aren", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Aren Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
