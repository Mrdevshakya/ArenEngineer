...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.AREN_STATE_DIR;
  const configPath = env.AREN_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    AREN_PROFILE: profile,
    AREN_STATE_DIR: stateDir,
    AREN_CONFIG_PATH: configPath,
    AREN_GATEWAY_PORT: String(port),
    AREN_GATEWAY_TOKEN: token,
    AREN_LAUNCHD_LABEL: resolvedLaunchdLabel,
    AREN_SYSTEMD_UNIT: systemdUnit,
    AREN_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    AREN_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    AREN_STATE_DIR: stateDir,
    AREN_CONFIG_PATH: configPath,
    AREN_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    AREN_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    AREN_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    AREN_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    AREN_LOG_PREFIX: "node",
    AREN_SERVICE_MARKER: NODE_SERVICE_MARKER,
    AREN_SERVICE_KIND: NODE_SERVICE_KIND,
    AREN_SERVICE_VERSION: VERSION,
  };
}
