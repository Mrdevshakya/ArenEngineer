...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AREN_GATEWAY_PORT).toBe("18789");
    expect(env.AREN_GATEWAY_TOKEN).toBe("secret");
    expect(env.AREN_SERVICE_MARKER).toBe("aren");
    expect(env.AREN_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AREN_SERVICE_VERSION).toBe("string");
    expect(env.AREN_SYSTEMD_UNIT).toBe("aren-gateway.service");
    if (process.platform === "darwin") {
      expect(env.AREN_LAUNCHD_LABEL).toBe("ai.aren.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AREN_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AREN_SYSTEMD_UNIT).toBe("aren-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.AREN_LAUNCHD_LABEL).toBe("ai.aren.work");
    }
  });
});
