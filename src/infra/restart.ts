...
export function triggerArenRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AREN_SYSTEMD_UNIT,
        process.env.AREN_PROFILE,
      );
  ...
  const label =
    process.env.AREN_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AREN_PROFILE);
