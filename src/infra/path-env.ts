...
function candidateBinDirs(opts: EnsureArenPathOpts): string[] {
  ...
  // Bundled macOS app: `aren` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "aren");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/aren` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "aren"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `aren` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureArenCliOnPath(opts: EnsureArenPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AREN_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.AREN_PATH_BOOTSTRAPPED = "1";
