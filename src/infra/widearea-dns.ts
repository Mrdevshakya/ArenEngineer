...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AREN_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aren-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "aren";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "aren");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "aren-gw");
  ...
  records.push(`_aren-gw._tcp IN PTR ${instanceLabel}._aren-gw._tcp`);
  records.push(`${instanceLabel}._aren-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aren-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aren-content-hash: ${contentHash}\n${contentBody}`;
}
