...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aren-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "aren doctor",
        managerScriptArgs(manager, "aren", ["doctor", "--non-interactive"]),
        gitRoot,
        { AREN_UPDATE_IN_PROGRESS: "1" },
      ),
    );
