...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AREN_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Aren";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Aren\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.AREN_MDNS_HOSTNAME?.trim() ||
    process.env.AREN_MDNS_HOSTNAME?.trim() ||
    "aren";
  const hostname =
    hostnameRaw
