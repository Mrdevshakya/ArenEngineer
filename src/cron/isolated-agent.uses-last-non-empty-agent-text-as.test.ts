...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "aren-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".aren", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ArenConfig> = {},
): ArenConfig {
  const base: ArenConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "aren"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".aren", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
