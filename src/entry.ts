...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.AREN_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.AREN_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.AREN_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[aren] ${parsed.error}`);
    process.exit(2);
  }
