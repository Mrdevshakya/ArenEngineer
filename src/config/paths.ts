...
import type { ArenConfig } from "./types.js";

/**
 * Nix mode detection: When AREN_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.AREN_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

const LEGACY_STATE_DIRNAMES = [".aren", ".aren", ".moldbot"] as const;
const NEW_STATE_DIRNAME = ".aren";
const CONFIG_FILENAME = "aren.json";
const LEGACY_CONFIG_FILENAMES = ["aren.json", "aren.json", "moldbot.json"] as const;
 ...
 * Can be overridden via AREN_STATE_DIR.
 * Default: ~/.aren
 */
 ...
 * Can be overridden via AREN_CONFIG_PATH.
 * Default: ~/.aren/aren.json (or $AREN_STATE_DIR/aren.json)
 */
...
export function resolveConfigPath(
  ...
  const stateOverride = env.AREN_STATE_DIR?.trim();
  const candidates = [
 ...
 * Default: os.tmpdir()/aren-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `aren-${uid}` : "aren";
 ...
 *
 * Precedence:
 * - `AREN_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.AREN_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
