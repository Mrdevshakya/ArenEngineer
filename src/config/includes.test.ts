...
const ETC_AREN_DIR = path.join(ROOT_DIR, "etc", "aren");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "aren.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcArenPath(...parts: string[]) {
  return path.join(ETC_AREN_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcArenPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "aren.json"))).toEqual({
      shared: true,
    });
  });
});
