  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.aren/logs/aren.log" },
        hooks: {
          path: "~/.aren/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".aren", "logs", "aren.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".aren", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".aren", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".aren", "agents", "main"));
