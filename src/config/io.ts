...
import type { ArenConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareArenVersions } from "./version.js";
...
function coerceConfig(value: unknown): ArenConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ArenConfig;
}
...
function stampConfigVersion(cfg: ArenConfig): ArenConfig {
...
function warnIfConfigFromFuture(cfg: ArenConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareArenVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Aren (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ArenConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ArenConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ArenConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ArenConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ArenConfig) {
    clearConfigCache();
...
// module scope. `AREN_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AREN_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AREN_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): ArenConfig {
...
export async function writeConfigFile(cfg: ArenConfig): Promise<void> {
  clearConfigCache();
