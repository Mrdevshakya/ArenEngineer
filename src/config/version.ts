...
export function parseArenVersion(raw: string | null | undefined): ArenVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareArenVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseArenVersion(a);
  const parsedB = parseArenVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
