...
import type { ArenConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ArenConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ArenConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ArenSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ArenConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ArenConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ArenConfig)),
    ),
  };
}
