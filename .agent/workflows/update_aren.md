---
description: Update Aren from upstream when branch has diverged (ahead/behind)
---

# Aren Upstream Sync Workflow
...
# Move old version
mv /Applications/Aren.app /tmp/Aren-backup.app

# Install new build
cp -R dist/Aren.app /Applications/

# Launch
open /Applications/Aren.app
```
...
ps aux | grep -E "(aren|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm aren agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update aren.json with fallback chains
...
pnpm aren agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
